...
{
    public partial class frmDispatch : frmBaseForm
    {
        ...
        public void setValues(int doc)
        {
            ...
            try
            {
                Program.NoMsg("");
                btnSubmit.Text = "&Add";
                TrnsDispatch trm = (from a in oDB.TrnsDispatch where a.DocNum == doc select a).FirstOrDefault();
                ...
                {
                    SaleOrderData oSO = new SaleOrderData();
        ...
        private string GetVoucherNumber()
        {
            ...
            try
            {
                Int64 value = Convert.ToInt64((from a in oDB.TrnsDispatch where a.DocNum == Convert.ToInt32(ActiveSeries) select a.DocNum).Max());
                if (value != 0)
                {
        ...
        private string GetDocNumber()
        {
            ...
            try
            {
                int value = Convert.ToInt32((from a in oDB.TrnsDispatch select a.DocNum).Max());
        ...
        private void LoadGrid()
        {
            try
            {
                dt.Clear();
                CreateDt();
                IEnumerable<TrnsDispatch> getData = from a in oDB.TrnsDispatch where a.FlgSecondWeight == false select a;
        ...
        public bool AddRecordNew()
        {
            try
            {
                ...
                oDoc.FUpdateDate = DateTime.Now;
                oDB.TrnsDispatch.InsertOnSubmit(oDoc);
                oDB.SubmitChanges();
        ...
        public bool UpdateRecordNew()
        {
            try
            {
                TrnsDispatch oDoc = (from a in oDB.TrnsDispatch
        ...
        public bool UpdateRecordSpecial()
        {
            try
            {
                ...
                var oDoc = (from a in oDB.TrnsDispatch
                            where a.DocNum.ToString() == txtDocNo.Text
                            select a).FirstOrDefault();
        ...
        private int ReturnDocNum()
        {
            ...
            try
            {
                ...
                {
                    int value = (from a in oprivate.TrnsDispatch
